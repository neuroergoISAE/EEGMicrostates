...
%% All the required settings for the microstates analysis
  %% Preparation: set paths etc. 
   ...
   settings.levels = {'session','participant','group'};
%     if settings.multipleSessions % pourrait être optimisé
...
%         p1path = dir([folders(1).folder,filesep, folders(1).name]);
%         settings.nSessions = length(p1path(contains({p1path.name},'ses')));  %calcule le nombre de session (sur p1, doit être le même pour tous les participants).
%     end
  %%  
...
%   %process path
%   settings.path.process=[settings.path.results,'process',filesep]; %intermediate output for each subject
%   if ~isfolder(settings.path.process) 
%     mkdir(settings.path.process); 
%   end
