...
for u = settings.microstate.Nmicrostates %loop over microstate models (~ number of clusters)
    
    %try
        %% Load Input Data
        disp('.. load data');
        if strcmp(level,settings.levels{1}) %first level : GFP as input
            ...
            CEEG = [];
            if  settings.multipleSessions %previous folder is one folder up
                ...
                previouslevelFolder = inputfolder;
            end
        ...
        EEG = pop_micro_segment( ...
            EEG, ...
            'algorithm',      settings.microstate.algorithm, ...
            'sorting',        settings.microstate.sorting, ...
            'normalise',      settings.microstate.normalise, ...
            'Nmicrostates',   settings.microstate.Nmicrostates, ... # range of microstate models
            'verbose',        settings.microstate.verbose, ...
            'Nrepetitions',   settings.microstate.Nrepetitions, ...
            'fitmeas',        settings.microstate.fitmeas, ...
            'max_iterations', settings.microstate.max_iterations, ...
            'threshold',      settings.microstate.threshold, ...
            'optimised',      settings.microstate.optimised);
